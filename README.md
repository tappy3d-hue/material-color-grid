...
3. Redoパネルでオプション調整：
   ...
   - **Group Name** — 新規グリッドのテクスチャ名・マテリアル名（デフォルト `ColorGrid`）。同名があれば自動で連番が付く
...
全オブジェクトをまたいでユニークな色だけが集約され、`ceil(√n) × ceil(n/cols)` のグリッドテクスチャ（例：4色 → 2×2、10色 → 4×3で2マス空き）が1枚だけ生成され、共有マテリアルが割り当てられます。テクスチャ名・マテリアル名は **Group Name** で指定でき、別グループのモデルには別名を付けることで上書きを防げます。**ジオメトリは結合されず、メッシュは分割されたままです。**
...
- 新規作成時のテクスチャ名・マテリアル名は **Group Name**（デフォルト `ColorGrid`）で指定できます。同名が既にある場合はBlenderが自動で連番（`.001` など）を付けるため、別グループを上書きしません。更新時は Group Name は無視され、選択オブジェクトが実際に使っているテクスチャが再利用されます。
